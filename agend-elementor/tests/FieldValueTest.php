...
final class FieldValueTest extends TestCase {
	...
	public function should_prefer_categories_array_over_category_object_when_both_present(): void {
		...
		$this->assertSame( 'Workshops', agend_elementor_field_value( 'event:category', 'event', $record ) );
	}

	#[Test]
	public function should_read_category_string_when_course_category_is_scalar(): void {
		$this->assertSame( 'Leadership', agend_elementor_field_value( 'course:category', 'course', array( 'category' => 'Leadership' ) ) );
	}

	#[Test]
	public function should_return_null_when_category_absent(): void {
		$this->assertNull( agend_elementor_field_value( 'event:category', 'event', array() ) );
	}

	#[Test]
	public function should_resolve_common_title_to_name_for_events_and_title_for_courses(): void {
		$this->assertSame( 'Gala', agend_elementor_field_value( 'common:title', 'event', array( 'name' => 'Gala' ) ) );
		$this->assertSame( 'Intro', agend_elementor_field_value( 'common:title', 'course', array( 'title' => 'Intro' ) ) );
	}
	...
	public function should_pick_member_price_when_viewer_is_member(): void {
		...
		$this->assertSame( 50, agend_elementor_field_value( 'event:price_from', 'event', $record ) );
	}
	...
	public function should_pick_non_member_price_when_viewer_is_anonymous(): void {
		...
		$this->assertSame( 80, agend_elementor_field_value( 'event:price_from', 'event', $record ) );
	}

	#[Test]
	public function should_return_null_when_field_does_not_apply_to_record_type(): void {
		$this->assertNull( agend_elementor_field_value( 'course:title', 'event', array( 'title' => 'x' ) ) );
		$this->assertNull( agend_elementor_field_value( 'nonsense:key', 'event', array() ) );
	}
	...
	public function should_not_warn_when_record_values_have_wrong_types(): void {
		...
		$this->assertNull( agend_elementor_field_value( 'event:start_date', 'event', $record ) );
		$this->assertNull( agend_elementor_field_value( 'event:category', 'event', $record ) );
		$this->assertNull( agend_elementor_field_value( 'event:price_member_from', 'event', $record ) );
	}
	...
	public function should_read_progress_percentage_and_default_to_zero_when_absent(): void {
		...
		$this->assertSame( 42.5, agend_elementor_field_value( 'course:progress_percent', 'course', $enrolled ) );
		$this->assertSame( 0, agend_elementor_field_value( 'course:progress_percent', 'course', array() ) );
	}
	...
	public function should_build_excerpt_from_stripped_description_when_no_short_description(): void {
		...
		$this->assertSame( 'Hello world again', agend_elementor_field_value( 'common:excerpt', 'event', $record ) );
	}
	...
	public function should_treat_free_course_as_zero_price(): void {
		$this->assertSame( 0, agend_elementor_field_value( 'course:price', 'course', array( 'is_free' => true, 'base_price' => 120 ) ) );
		$this->assertSame( 120, agend_elementor_field_value( 'common:price_from', 'course', array( 'base_price' => 120 ) ) );
	}

	#[Test]
	public function should_use_detail_url_from_extra_when_provided(): void {
		$value = agend_elementor_field_value( 'common:detail_url', 'event', array( 'slug' => 'x' ), array( 'detail_url' => 'https://example.test/events/event/x/' ) );
	...
	public function should_group_picker_options_and_restrict_by_kind(): void {
		$all    = agend_elementor_field_options();
		$labels = array_column( $all, 'label' );
		$urls   = agend_elementor_field_options( array( 'url' ) );
		...
		foreach ( $urls as $group ) {
			foreach ( array_keys( $group['options'] ) as $key ) {
				$this->assertSame( 'url', agend_elementor_field_kind( $key ) );
			}
		}
	}
	...
	public function should_declare_a_known_kind_for_every_field(): void {
		foreach ( agend_elementor_field_registry() as $key => $descriptor ) {
			$this->assertContains( $descriptor['kind'], AGEND_ELEMENTOR_FIELD_KINDS, $key );
