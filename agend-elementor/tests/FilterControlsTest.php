 ...
 */
final class Filter_Controls_Test_Harness extends Agend_Elementor_Filter {
 ...
 * `choices` REPEATER the adapter still declares itself.
 */
#[CoversClass( Agend_Elementor_Schema_Controls::class )]
final class FilterControlsTest extends TestCase {
	...
	public function should_register_the_same_content_tab_controls_as_the_pre_refactor_widget(): void {
		$fixture = json_decode(
			file_get_contents( __DIR__ . '/fixtures/filter-content-controls.json' ),
			true
		);

		$widget = new Filter_Controls_Test_Harness();
		$widget->run();
