...
if ( ! class_exists( 'Agend_Directory_Sync_Http_Api_Source' ) ) :
	final class Agend_Directory_Sync_Http_Api_Source implements Agend_Directory_Sync_Source {
		...
		/**
		 * Whether a URL is allowed: absolute HTTPS always; absolute HTTP
		 * only when `wp_get_environment_type()` is `local` or `development`
		 * (US-2.1 criterion 6).
		 */
		 ...
		 * object), as opposed to a list or a scalar. An empty array is
		 * treated as non-associative (skipped): `json_decode('{}', true)`
		 * and `json_decode('[]', true)` are both PHP `array()`, so an empty
		 * object is indistinguishable from an empty list here — but either
		 * way it carries no fields to transform.
		 *
		 * @param mixed $value
		 */
		...
		/**
		 * Sanitise a URL setting: trim, run through esc_url_raw(), and drop
		 * it entirely (return '') if it is not an absolute HTTPS URL (or an
		 * allowed local/development HTTP URL) — US-2.1 criterion 6,
		 * US-2.4 criterion 2. Blanking rather than persisting an invalid
		 * value means is_available() reports the plain "no URL configured"
		...
		/**
		 * Sanitise a query-parameter name setting to
		 * `[A-Za-z0-9_.\[\]-]` (US-2.4 criterion 2), falling back to the
		 * given default when the sanitised result is blank.
		 */
		...
		/**
		 * Sanitise a dot-path setting (response data path / has-more path)
		 * to the same character set the field map's source paths allow, so
		 * nested paths remain expressible.
		 */
